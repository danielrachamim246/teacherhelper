...
def handle_snaps_server():
	...
	while True:
		...
		userid = arr_data[0]
		snapid = arr_data[1]
		print 'From userid: {0}, and snapid: {1}'.format(userid, snapid)
...
def handle_client(client_sock, addr, userid):
	...
	"""
	print '[*] New Thread for {0}:{1}!'.format(addr[0], addr[1])
	while True:
		req = client_sock.recv(9999999)
		print '[*] Client {0}:{1} requests {2}'.format(addr[0], addr[1], req)
		# Get a job and send
...
def main():
	...
	while True:
		...
		print 'New client from {0}:{1}, userid={2}'.format(addr[0], addr[1], userid)
